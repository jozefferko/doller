...
result = pd.read_sql_query("SELECT PRODTABLE.NAME,JMGSTAMPTRANS.JOBREF,JMGSTAMPTRANS.CORRSTARTTIME, JMGSTAMPTRANS.CORRSTOPTIME,JMGSTAMPTRANS.QTYGOOD,JMGSTAMPTRANS.CORRSTARTDATE,JMGSTAMPTRANS.CORRSTOPDATE from JMGSTAMPTRANS inner join PRODTABLE on PRODTABLE.PRODID=JMGSTAMPTRANS.JOBREF WHERE  PRODTABLE.NAME LIKE '%ST1 :%' AND CORRSTARTDATE BETWEEN '2018-01-01' AND '2019-01-01'",getData())
result2 = pd.read_sql_query("SELECT [Date], [Time],[Indgang 0101],[Indgang 0102],[Indgang 0103],[Indgang 0104],[Indgang 0105] ,[Indgang 0106] FROM [Indusoft].[dbo].[DataCollection] WHERE [Date] BETWEEN '2017-01-01' AND '2019-01-01'", getData())
...
result.to_csv("PRODTABLE.csv", sep=";")
result2.to_csv("DataCollection.csv", sep=";")
